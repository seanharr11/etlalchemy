...
class SchemaTransformer():
    ...
    class TableTransformation():
        ...
        def __str__(self):
            return "({0} -> {1}...Delete = {2})".\
                format(self.oldTable, self.newTable, str(self.delete))
        ...
        if column_transform_file:
            with open(column_transform_file, "rU") as fp:
                ...
                for row in dr:
                    ...
                    if not self.columnTransformations.get(st.oldTable):
        ...
        if thisTableTT:
            ...
            if thisTableTT.newTable not in ["", None]:
                self.logger.info(
                    ...
                    .format(table.name, thisTableTT.newTable))
            ...
            for k in self.global_renamed_col_suffixes.keys():
                ...
                if initialColumnName.lower().endswith(k.lower()):
                    self.logger.info(
                        ...
                        .format(initialColumnName.lower(),
                                initialColumnName.replace(
                                    k, self.global_renamed_col_suffixes[k]),
                                k.lower()))
                    C.name = initialColumnName.replace(
                            k, self.global_renamed_col_suffixes[k])
                    columns[idx] = C.name
        return columns
                    ...
                    try:
                        if st.action.lower() == "rename":
                            if thisTableST.get(st.newColumn):
                                if thisTableST[st.newColumn].action.lower() ==\
                                        "delete":
